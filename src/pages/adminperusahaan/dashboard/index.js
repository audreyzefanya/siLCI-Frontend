...
import { GetDashboardAdminImpor } from '../../../service/dashboard/endpoint';
...
const Dashboard = (props) => {
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Jumlah Permintaan Barang Impor 30 Hari Terakhir',
      },
      scales: {
        y: {
            type: 'linear',
  ...
  const fetchDetails = async (userInfo) => {
    try {
        const response = await GetDashboardAdminImpor(userInfo.id);
    ...
    <div className='flex w-screen h-screen'>
        ...
        <div className='w-full h-screen flex flex-col'>
            ...
            <div className="flex justify-between mx-10">
               <div style={{ backgroundColor: '#2C358C', color: 'white', padding: '20px', borderRadius: '10px', textAlign: 'center', width: '22.5%' }}>
                    <h2>Jumlah Barang Terdaftar</h2>
                    <p style={{ fontSize: '36px', fontWeight: 'bold' }}>{dashboardDetail.jumlah_barang}</p>
                </div>
                <div style={{ backgroundColor: '#2C358C', color: 'white', padding: '20px', borderRadius: '10px', textAlign: 'center', width: '22.5%' }}>
                ...
                <div style={{ backgroundColor: '#2C358C', color: 'white', padding: '20px', borderRadius: '10px', textAlign: 'center', width: '22.5%' }}>
                    <h2>Jumlah Permintaan Aktif</h2>
                    <p style={{ fontSize: '36px', fontWeight: 'bold' }}>{dashboardDetail.jumlah_pengadaan_aktif}</p>
                </div>
                <div style={{ backgroundColor: '#2C358C', color: 'white', padding: '20px', borderRadius: '10px', textAlign: 'center', width: '22.5%' }}>
                    <h2>Sedang Menunggu Persetujuan</h2>
                    <p style={{ fontSize: '36px', fontWeight: 'bold' }}>{dashboardDetail.jumlah_pengadaan_requested}</p>
                </div>
            </div>
            <br></br>
            {chartData && (
